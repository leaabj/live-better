...
export function createTasksRouter(db = defaultDb) {
  ...
  tasksRouter.get("/all", authMiddleware, async (c) => {
    try {
      ...
      return c.json(
        { success: false, error: "Failed to fetch all tasks" },
        500,
      );
    }
  });
          ...
          return c.json(
            {
              success: false,
              error:
                "specificTime must be a Date object or ISO timestamp string",
            },
            400,
          );
        }
  ...
  tasksRouter.post("/:id/validate-photo", authMiddleware, async (c) => {
    try {
      ...
      // Verify task exists and belongs to user (check this before service configuration)
      const existingTask = await db
        .select()
        .from(tasks)
        .where(and(eq(tasks.id, id), eq(tasks.userId, user.userId)));
      ...
      // Parse form data with image (check this before service configuration)
      ...
      // Check if photo validation is configured (check this after basic validation)
      if (!PhotoValidationService.isConfigured()) {
        return c.json(
          {
            success: false,
            error: "Photo validation service is not configured",
          },
          503,
        );
      }
